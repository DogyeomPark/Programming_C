...
//#include <math.h>
//
//#pragma warning (disable:4996) //4996번 오류 메세지 미사용
//
//#define ESC 27
//#define MAXENEMY 10
//#define MAXBALL 10
...
//
//	hConsole = GetStdHandle(STD_OUTPUT_HANDLE); //커서 보이게 혹은 안보이게 하는것
//
//	ConsoleCursor.bVisible = show; //bVisible 은 ActiveSelf랑 비슷함.
...
//	system("cls"); //clear screen 의 약자
//	CursorView(0);
//
//	for (count = 0; ; count++) //조건을 뺌
//	{
//		if (count % 2 == 0)
//		{
...
int main()
{
	...
	{
		// 좌우 이동처리
		if (count % 2 == 0) // count를 올리면서 중간에 한번씩 체크하기 위한 용도
		{
			if (IsKeyDown(VK_LEFT)) //만약 왼쪽 화살표가 눌렸다면...
			{
				if (fx > 6)
					fx--;
			}

			if (IsKeyDown(VK_RIGHT)) //만약 오른쪽 화살표가 눌렸다면...
			{
				if (fx < 72)
					fx++;
			}
		...
		if (rand() % 50 == 0)
		{
			...
			if (i != MAXENEMY)
			{
				...
				Enemy[i].nFrame = Enemy[i].nStay = rand() % 6 + 1;
				Enemy[i].Type = rand() % (sizeof(arEnemy) / sizeof(arEnemy[0])); // 랜덤 적기 타입 
